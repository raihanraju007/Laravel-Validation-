<?php

use Illuminate\Support\Facades\Route;
use App\Http\Controllers\FormController;


/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::get('/', function () {
    return view('welcome');
});


Route::get('form/client', [FormController::class, 'ClientSideFormValidation'])->name('client.form.validation');
Route::post('form/client/store', [FormController::class, 'ClientStore'])->name('client.store');


Route::get('form/server', [FormController::class, 'ServerSideLaravelValidation'])->name('server.laravel.validation');
Route::post('form/server/store', [FormController::class, 'ServerStore'])->name('server.store');

